...
| Hash-chain integrity | Periodically re-derive the chain with `firstpass verify` (exits 1 if broken, so it drops straight into a cron/CI check; `--file` re-derives from an exported JSONL with no proxy or DB in the loop). `uvx firstpass demo` shows the same pattern end to end: it drives a decision, then re-verifies the sealed chain | Verifies clean every time. A verification failure is a Sev-1 — it means the trace file was tampered with or corrupted |
