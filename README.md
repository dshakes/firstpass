...
- 🎯 **Predict-to-start, verify-to-serve** — a learned start-rung bandit (deterministic UCB1, or Thompson sampling with drift-forgetting and logged propensities) picks where the ladder *starts*; the gate still checks the output before it ships.
...
**v0.2.0 — pre-GA, shipped in the open.** Working today: enforce + observe on **both wire dialects** (Anthropic `/v1/messages` + OpenAI `/v1/chat/completions`, both live-verified end-to-end); structured (tools/images) enforce **default-on** with verbatim per-rung body carry and a fidelity guard; streaming clients get an instant SSE connection with keepalives while the ladder routes; cross-provider failover; schema + subprocess + LLM-judge + self-consistency gates with per-gate `on_abstain` policy and gate cost priced onto every receipt; learned start-rung (UCB1 or discounted Thompson with logged propensities, ADR 0007) with a speculative-deferral band; the earned conformal guarantee ([committed artifact](docs/benchmarks/)) plus Learn-then-Test calibration (`firstpass calibrate --method ltt`) and the live adaptive threshold; policy rehearsal with IPS/SNIPS/DR (`firstpass ope`); `firstpass savings` and `firstpass evals` from your own receipts (also MCP tools); durable receipts (`FIRSTPASS_RECEIPTS=durable` — never drop, chain-valid recovery); per-deployment `[[price]]` overrides; a committed Grafana dashboard; nightly per-provider live smoke in CI. Honest limits, tracked on the [roadmap](docs/roadmap.md): cross-dialect structured translation beyond Anthropic↔OpenAI is next; four provider dialects await live wire verification (smoke jobs activate when repo secrets are set); 30-day soak, external security audit, and the hosted multi-tenant plane are ahead of us, not behind us. GA is a checklist we publish ([ADR 0003](docs/adr/0003-ga-readiness.md)), not an adjective — the exact remaining items (secrets, soak clock, external audit) are enumerated in the [GA handoff](docs/ga-handoff.md).
