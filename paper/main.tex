...
headline result holds by degrading the gate deliberately, and find \result{one-sentence summary of
the sweep finding: at which $\tau$ false-accepts appear and whether the conformal bound stays
feasible}.
\end{abstract}
...
\paragraph{Setup.} MBPP, $n = \result{task count}$ tasks. Ladder:
\result{ladder model ids}. The gate is the visible unit-test prefix; ground truth is a disjoint
...
\midrule
always-cheap  & \result{} & \result{} & --- & --- \\
always-top    & \result{} & \result{} & --- & --- \\
first-pass    & \result{} & \result{} & \result{} & \result{} \\
\bottomrule
\end{tabular}
\caption{Paired over identical tasks. Paired $\Delta$ vs always-top:
\result{delta and 95\% CI}.}
\end{table}
...
\midrule
Zero Router (models only)   & \result{} \\
{}+ first-pass              & \result{} \\
Oracle (perfect foresight)  & \result{} \\
\bottomrule
\end{tabular}
\caption{AIQ lift over the Zero Router: \result{lift}, closing \result{fraction} of the headroom to
perfect foresight.}
\end{table}
...
\midrule
1.00 & \result{} & \result{} & \result{} & \result{} & \result{} \\
\multicolumn{6}{c}{\result{remaining sweep rows from the artifact}} \\
\bottomrule
\end{tabular}
\caption{Gate AUC (score $\rightarrow$ oracle-correct): \result{}. Split-conformal $\lambda$ =
\result{}, held-out realized served-failure \result{} against $\alpha = 0.10$.}
\end{table}

RouterBench reports that cascading routers beat the Zero Router while judge error stays at or below
$0.1$, degrading sharply past $0.2$. Our sweep locates our own gate on that axis:
\result{where the measured gate error sits relative to 0.1/0.2}.
...
\midrule
first-pass     & \result{} & \result{} & --- & --- \\
meta-verified  & \result{} & \result{} & \result{} & \result{} \\
\bottomrule
\end{tabular}
\caption{Held-out comparison. We expect no advantage where the gate is a near-perfect oracle; the
question is whether one appears as $\tau$ degrades.}
\end{table}
