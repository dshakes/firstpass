...
- **Any provider · open-source routing ✓** — beyond the built-in Anthropic + OpenAI, a `[[provider]]` entry adds any OpenAI-compatible host (Groq, Together, Fireworks, DeepSeek, Mistral, xAI, OpenRouter, Azure) or a local **Ollama / vLLM** server. A ladder rung is `<id>/<model>`, so a route can open on a cheap open-source model and escalate to a frontier model only on gate failure. (Bespoke-auth providers — Bedrock SigV4, Vertex — are the named follow-on.)
